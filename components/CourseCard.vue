<template>
  ...
  >
    <div class="aspect-[16/10] bg-gray-100 relative overflow-hidden">
      <img
        v-if="course.imagem"
        :src="resolveMediaUrl(course.imagem)"
        :alt="course.titulo"
      ...
      <span
        v-if="course.modalidade"
        class="absolute top-3 right-3 text-xs font-medium px-3 py-1 rounded-full backdrop-blur-sm"
        :class="course.modalidade === 'ead' ? 'bg-accent/90 text-white' : 'bg-white/90 text-primary'"
      >
        {{ modalidadeLabel }}
      </span>

      <span
        v-if="course.inscricao_status"
    ...
    <div class="p-5 flex flex-col flex-1">
      ...
      <div class="flex flex-wrap gap-2 text-xs mb-4">
        <span v-if="course.carga_horaria" class="bg-surface text-text px-2.5 py-1 rounded-lg">{{ course.carga_horaria }}</span>
      </div>
...
interface CourseCardProps {
  course: {
    ...
    vagas_totais?: number
    inscricao_status?: 'disponivel' | 'esgotado' | 'encerrada'
    imagem?: string | null
    partner?: { name: string }
  }
}
...
const trilhaLabel = computed(() => trilhaLabels[props.course.trilha || ''] || props.course.trilha || '')
const trilhaGradient = computed(() => trilhaGradients[props.course.trilha || ''] || 'bg-gradient-to-br from-primary/30 to-accent/30')
const modalidadeLabel = computed(() => {
  if (props.course.modalidade === 'ead') return 'EAD'
  if (props.course.modalidade === 'presencial') return 'Presencial'
  return props.course.modalidade || ''
})
</script>
